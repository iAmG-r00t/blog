...
Hi, 👋 happy new year 🎉.  In my previous [post](2019-04-17-rabbit-hole-adventures-appsec-teaser.md), I talked about the lessons I learned while learning how to find bugs in a mobile application.
...
In this post, I am going to focus on the mobile application and its challenges.

The players were provided an APK file. When I got hold of the APK file, I installed it on my test device, a Homepesa Sacco application that had a login and register account when spawned.

There were six challenges, so before starting to solve the challenges I decided to do some recon to get to understand the application.

## Recon

Below are the steps I normally follow to get to understand the application;
...
2. **Read the manifest file** to identify application-defined permissions, look for any misconfigurations set, locate any exported activities or services, and identify any broadcast receivers and content providers.
3. **Used [drozer](https://labs.f-secure.com/tools/drozer/)** to identify any attack surface present, and look for files that are being stored locally in the application directory.
...
From the title of the challenge, it seemed like we were to take a look at the manifest file but from my recon process, I never found it. After consulting with the creator he said the flag was a comment in the manifest file. For this challenge one couldn't attain the flag because it wasn't there, it seems like when one is building the application comments that were placed in the manifest file were removed.
...
For the second challenge, I went ahead on registering an account, before finishing registration one was required to do a registration payment. For this challenge when I was reviewing the code I saw the flag right there hardcoded in the `Home` class.
...
Most of the players went ahead and submitted this flag, but for me, I wasn't satisfied. This wasn't the proper way of attaining the flag. So I went ahead and reviewed the code. A database file called `bitwise` was being created when the **onCreate** method is called, and data was being placed in the database during the account creation process.
...
Looking at the Alert statement, it was being triggered after checking the `bitwise` database if the account status was active or not active and if an account is active it's when we are presented with our flag.
...
This was a very interesting challenge. I learned a new attack vector before the event was about to end, the challenge had zero solves so we were given a clue, try to log in with a correct email address but a wrong password. I went ahead and did that and I was presented with an alert box saying that I have entered the wrong password, prompting me if I would want to recover the password. So I proceeded and placed my email on the recover password page and was sent a recovery link in the email address I provided.
...
Looking at the URL link; `http://159.203.60.168/recoverypassmy.php?id=9408` seems like we can test for SQL injection, I proceeded to test and my tool of choice was [sqlmap](https://github.com/sqlmapproject/sqlmap).
...
Boom we have an SQL injection present, and we have our entry point **`recoverpassmy.php`** which is our flag.
...
This will be short, trust me .... so md5 hash isn't the best encryption. The best thing about sqlmap is that it's capable of decrypting/cracking hashes with either a custom (which you feed it ...) or default wordlist. But there are also other ways of decrypting md5 hashes.
...
Obtaining the flag for the last challenge was tough. I stared at the code for a while and the way I found the flag was more on checking each class that was involved in sending SMS.
...
When I saw this, I knew that I am close. So I decided to look at the **`MySMSBroadcastReceiver`** class where there was no interesting thing so after sitting down and looking at the **`MobilesasaClient`** class I noticed a small detail I had missed **`sendSMS`** I used that to query for any other string that contains the same string and I found the flag in the **`ConfirmPhone`** class.
...
As we can note the flag states we should have done network analysis to solve the challenge, next time I will have that in mind.

## Epilogue

I enjoyed solving the challenges where I got the opportunity to learn something new. I would love to thank the creator [@shellcode254](https://twitter.com/shellcode254) and the [@AfricaHackon](https://twitter.com/AfricaHackon) team.
